...
/*   Updated: 2021/10/21 09:55:20 by cdiaz-fl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include	"push_swap.h"
...
void	ft_r(t_param *val, char *c)
{
	...
	if (val->size_a > 1 && val->size_b > 1)
	{
		if ((!ft_strncmp("ra", c, 2) && val->head_b->content < val->tail_b->content) ||
			(!ft_strncmp("rb", c, 2) && val->head_a->content > val->tail_a->content))
			c = "rr";
	}
...
void	ft_rr(t_param *val, char *c)
{
	...
	if (val->size_a > 1 && val->size_b > 1)
	{
		if ((!ft_strncmp("rra", c, 3) && val->head_b->content < val->tail_b->content) ||
			(!ft_strncmp("rrb", c, 3) && val->head_a->content > val->tail_a->content))
			c = "rrr";
	}
